        ...
        QObject(parent)
{
    ...
    //struct PortSettings hw_ComSetting={BAUD1151000,DATA_8,PAR_NONE,STOP_1,FLOW_OFF,1000};
    ...
    hw_Com->setBaudRate(BAUD115200); 	//波特率设置，我们设置为1151000
    ...
    aliveTimer->start(10000); // 10s查询一次在线情况
...
bool HARDWARE::stm32IsAlive(void)//检测单片机读头在线情况
{
    ...
    if (!recv_data.isEmpty())
    {
        ...
        for (u_int i = 0; i < recv_len; i++)//转成16进制字符数组
        {
            ...
            if (CRC16((unsigned char *)recv_data_ascii, recv_len) == 0)//CRC校验
            {
                if (recv_data_ascii[2] == FUNCTION_CODE_STM32_IS_ALIVE)
                {
                    ...
                    if (this->hw_info.USE_MINSHENG_READER != MinSheng_Reader_ALIVE
                       ...
                       || this->hw_info.USE_485 != RS485_ALIVE)
                        qDebug("MS: %d  ID: %d  IC: %d  485: %d", MinSheng_Reader_ALIVE, Identity_Reader_ALIVE, IC_Reader_ALIVE, RS485_ALIVE);
                    #endif
                }
            }
        }
    ...
    emit sig_Use_Reader_Type(MinSheng_Reader_ALIVE, Identity_Reader_ALIVE, IC_Reader_ALIVE, RS485_ALIVE);
...
void HARDWARE::checkAlive()
{
    ...
    if (STM32_INFO_SYNC_FLAG == false)
    {
        ...
        if (this->hw_info.LIGHT_SENSOR_VALUE != 0xff)
        {
            ...
            qDebug() << "STM32 Version:" << Stm32_Version;
            ...
            qDebug() << "USE_485:" << this->hw_info.USE_485;
            ...
            emit sig_Use_Reader_Type(hw_info.USE_MINSHENG_READER, hw_info.USE_IDENTITY_READER, hw_info.USE_IC_READER, hw_info.USE_485);
            qDebug() << "get pwm:" << this->getInfraredPWM();
            qDebug() << "get light:" << this->getLinghtSensor_Status();
        }
        //return ;
    }
...
QString HARDWARE::asciiToQString(u_int8_t *recv_data, u_int32_t recv_len) // 16进制字符串转QString
{
    QString str;
    for (u_int32_t i = 0; i < recv_len; i++)
    {
...
char HARDWARE::ConvertHexChar(char ch) //将1-9 a-f字符转化为对应的整数
{
    if ((ch >= '0') && (ch <= '9'))
        return ch-0x30;
    else if ((ch >= 'A') && (ch <= 'F'))
    ...
    if (plen == (u_int)((pbuff[0] << 8) + pbuff[1]))
    {
        if (cmd != FUNCTION_CODE_A83T_IDENTITY_CARD)
        {
            ...
            case FUNCTION_CODE_A83T_LIGHT_SENSOR:   			//STM32上传光敏状态
            {
                ...
                if (pbuff[4] == 0x00)
                this->hw_info.LIGHT_SENSOR_VALUE = false;
                else if (pbuff[4] == 0x01)
                this->hw_info.LIGHT_SENSOR_VALUE = true;

                emit sig_LightSensor_Data(this->hw_info.LIGHT_SENSOR_VALUE);
            }
                break;
